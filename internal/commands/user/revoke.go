...
func (cmd *CommandRevoke) Handler(profile *cli.Profile, ui terminal.UI) error {
	...
	filter := realm.UserFilter{
		...
		State:     cmd.inputs.State,
		Pending:   cmd.inputs.Pending,
		Providers: realm.NewAuthProviderTypes(cmd.inputs.ProviderTypes...),
	}
	foundUsers, findErr := cmd.realmClient.FindUsers(app.GroupID, app.ID, filter)
	if findErr != nil {
		return findErr
	}
	if len(cmd.inputs.Users) > 0 && len(foundUsers) == 0 {
		return errors.New("no users found")
	}
	users, usersErr := selectUsers("revoke", ui, foundUsers)
	if usersErr != nil {
		return usersErr
	}
